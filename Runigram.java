...
public class Runigram {
	...
	private static Color luminance(Color pixel) {
		...
		Color lumin = new Color(lum, lum, lum);
		return lumin;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double sorceWidth = (double)image[0].length / width;
		double sorceHeight = (double)image.length / height;
		for (int i = 0; i < scaledImage.length; i++){
			for(int j = 0 ; j < scaledImage[i].length; j++){
				int sorceW = (int)(j * sorceWidth);
				int sorceH = (int)(i * sorceHeight);
				scaledImage[i][j] = image [sorceH][sorceW];
			}
		}
		return scaledImage;
	}
